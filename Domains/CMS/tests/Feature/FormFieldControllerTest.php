...
test('a field can be created, listed, updated, and deleted on its form', function () {
    $response = $this->postJson("/api/v1/forms/{$this->form->id}/fields", [
    ...
    $this->getJson("/api/v1/forms/{$this->form->id}/fields")->assertOk()->assertJsonCount(1, 'data');

    $this->putJson("/api/v1/forms/{$this->form->id}/fields/{$id}", ['label' => 'Your Email'])
        ->assertOk()
        ->assertJsonPath('data.label', 'Your Email');

    $this->deleteJson("/api/v1/forms/{$this->form->id}/fields/{$id}")->assertNoContent();
...
test('a field belonging to a different form is not found', function () {
    ...
    $this->getJson("/api/v1/forms/{$this->form->id}/fields/{$field->id}")->assertNotFound();
    $this->putJson("/api/v1/forms/{$this->form->id}/fields/{$field->id}", ['label' => 'Hijacked'])->assertNotFound();
    $this->deleteJson("/api/v1/forms/{$this->form->id}/fields/{$field->id}")->assertNotFound();
});
...
test('reordering rejects a field id that does not belong to the target form', function () {
    ...
    $this->patchJson("/api/v1/forms/{$this->form->id}/fields/reorder", [
...
test('a user without permission cannot manage form fields', function () {
    ...
    $this->postJson("/api/v1/forms/{$this->form->id}/fields", [
        'label' => 'Email', 'name' => 'email', 'type' => 'email',
    ])->assertForbidden();
});
