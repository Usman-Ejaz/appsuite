...
test('a blog can be created, listed, updated, and deleted', function () {
    $response = $this->postJson('/api/v1/cms/blogs', ['title' => 'Hello World', 'slug' => 'hello-world']);
    $response->assertCreated()->assertJsonPath('data.title', 'Hello World');

    $id = $response->json('data.id');

    $this->getJson('/api/v1/cms/blogs')->assertOk()->assertJsonCount(1, 'data');

    $this->putJson("/api/v1/cms/blogs/{$id}", ['title' => 'Updated Title'])
        ->assertOk()
        ->assertJsonPath('data.title', 'Updated Title');

    $this->deleteJson("/api/v1/cms/blogs/{$id}")->assertNoContent();
    $this->assertDatabaseMissing('blogs', ['id' => $id]);
});

test('creating a blog requires a title and slug', function () {
    $this->postJson('/api/v1/cms/blogs', [])->assertUnprocessable();
});
...
test('assigning another company\'s author or category is rejected', function () {
    ...
    $this->postJson('/api/v1/cms/blogs', [
        'title' => 'Hello World',
        'slug' => 'hello-world',
...
test('a blog belonging to a different company is not found', function () {
    ...
    $this->getJson("/api/v1/cms/blogs/{$blog->id}")->assertNotFound();
    $this->putJson("/api/v1/cms/blogs/{$blog->id}", ['title' => 'Hijacked'])->assertNotFound();
    $this->deleteJson("/api/v1/cms/blogs/{$blog->id}")->assertNotFound();
});
...
test('a user without permission cannot manage blogs', function () {
    ...
    $this->postJson('/api/v1/cms/blogs', ['title' => 'Hello World', 'slug' => 'hello-world'])->assertForbidden();
});
...
test('publishing a blog sets published_at once and republishing does not reset it', function () {
    ...
    $this->postJson("/api/v1/cms/blogs/{$blog->id}/publish")->assertOk();
    $firstPublishedAt = $blog->fresh()->published_at;

    expect($firstPublishedAt)->not->toBeNull();

    $this->postJson("/api/v1/cms/blogs/{$blog->id}/unpublish")->assertOk();
    $this->postJson("/api/v1/cms/blogs/{$blog->id}/publish")->assertOk();
...
test('a user without the publish permission cannot publish a blog', function () {
    ...
    $this->postJson("/api/v1/cms/blogs/{$blog->id}/publish")->assertForbidden();
});

test('slug uniqueness is scoped per company', function () {
    Blog::factory()->create(['company_id' => $this->company->id, 'slug' => 'hello-world']);

    $this->postJson('/api/v1/cms/blogs', ['title' => 'Another', 'slug' => 'hello-world'])
        ->assertUnprocessable();
});
...
test('author and category are nested when loaded', function () {
    ...
    $this->getJson("/api/v1/cms/blogs/{$blog->id}")
        ->assertOk()
